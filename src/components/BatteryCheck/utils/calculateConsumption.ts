...
export const calculateBatteryUsage = (data: BatteryDataEntry[]): Academy[] => {
    ...
    const schools: { [key: number]: Academy } = {};
    ...
    const schoolArray: Academy[] = Object.values(schools);
...
const calculateUsage = (items: BatteryDataEntry[]): { totalBatteryUsage: number; count: number } => {
    let totalBatteryUsage = 0;
    let count = 0;

    for (let x = 1; x < items.length; x++) {
        const previousItem = items[x - 1]
            , currentItem = items[x];

        // Check if battery level increased, if yes, skip (as mentioned in the assignment)
        if (currentItem.batteryLevel > previousItem.batteryLevel) continue;
        count++;
